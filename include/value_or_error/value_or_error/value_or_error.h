...
#include "value_or_error/helpers.h"
...
struct TraitsBase : public VariantStorage<Stored...> {
 public:
  ...
  using ConstructorRefSelector = ReferenceSelector<Ref, CopyConstructorArray, MoveConstructorArray, FromVoid>;
...
template <typename... Types>
using DestructorImpl =
    DestructorHolder<(... && (std::is_trivially_destructible_v<Types> || std::is_same_v<void, Types>)), Types...>;
...
struct ConstructorsImpl : public ValueConstructorImpl<ValueType, ErrorTypes...> {
 ...
 protected:
  ...
  void Construct(From&& from) {
    ...
    Base ::template ConstructorRefSelector<decltype(from), util::hlp::propagateConst<From, void>>::Call(
        from.Data(), Base::Data(), Base::PhysicalIndex());
  }
  ...
  void ConvertConstruct(From&& from, ConstructorsImpl<FromValueType, FromErrorTypes...>*) {
    ...
    FromType::template ConstructorRefSelector<decltype(from), util::hlp::propagateConst<From, void>>::Call(
        from.Data(), Base::Data(), from.PhysicalIndex());
...
struct AssignmentsImpl : public ConstructorsImpl<ValueType, ErrorTypes...> {
  ...
  void Assign(Assignee&& rhs) & noexcept {
    ...
    if (Base::LogicalIndex() == rhs.LogicalIndex()) {
      Base::template AssignmentRefSelector<decltype(rhs), util::hlp::propagateConst<Assignee, void>>::Call(
          rhs.Data(), Base::Data(), Base::PhysicalIndex());
      return;
    }

    Base::Clear();
    Base::LogicalIndex() = rhs.LogicalIndex();
    Base::template ConstructorRefSelector<decltype(rhs), util::hlp::propagateConst<Assignee, void>>::Call(
        rhs.Data(), Base::Data(), Base::PhysicalIndex());
  }
  ...
  void ConvertAssign(Convert&& rhs, ValueOrError<FromValueType, FromErrorTypes...>*) & noexcept {
    ...
    if (Base::PhysicalIndex() == this_phys_index) {
      RhsType::template AssignmentRefSelector<decltype(rhs), util::hlp::propagateConst<Convert, void>>::Call(
          rhs.Data(), Base::Data(), rhs_phys_index);
      return;
    }

    Base::Clear();
    Base::LogicalIndex() = Base::PhysicalToLogicalIndex(this_phys_index);
    RhsType::template ConstructorRefSelector<decltype(rhs), util::hlp::propagateConst<Convert, void>>::Call(
        rhs.Data(), Base::Data(), rhs_phys_index);
  }
};
...
struct DiscardErrorImpl : public AssignmentsImpl<ValueType, ErrorTypes...> {
  ...
  using ResultType =
      TransferTemplate<list::set::subtract<ErrorTypesList, list::list<DiscardedErrors...>>, ValueOrError, ValueType>;
...
class [[nodiscard]] ValueOrError : public detail::ValueOrErrorImpl<ValueType, ErrorTypes...> {
 ...
 public:
  ...
  static_assert(util::list::set::isaSet<util::list::list<ErrorTypes...>>, "Error types must not contain duplicates");
