...
#include "utility/type_list/front.h"
...
struct Filter<list<T, Ts...>, Predicate> {
  ...
  using type = std::conditional_t<Predicate<T>::value, typename Concat<list<T>, rest>::type, rest>;
};
...
struct Unique<list<T, Ts...>> {
  ...
  using type = std::conditional_t<Contains<list<Ts...>, T>::value, rest, typename Concat<list<T>, rest>::type>;
};
...
struct Intersect2<TL1, list<T, Ts...>> {
  ...
  using type = std::conditional_t<Contains<TL1, T>::value, typename Concat<rest, list<T>>::type, rest>;
};
...
struct Subtract<list<T, Ts...>, TL> {
  ...
  using type = std::conditional_t<Contains<TL, T>::value, rest, typename Concat<rest, list<T>>::type>;
};
...
struct IndexOf<T, list<U, Ts...>> : std::integral_constant<size_t, 1 + IndexOf<T, list<Ts...>>::value> {};
