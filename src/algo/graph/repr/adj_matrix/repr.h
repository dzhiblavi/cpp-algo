...
#include "algo/graph/repr/storage_type.h"
...
class AdjMatrixGraph {
 public:
  auto edgesFrom(NodeId from) noexcept {
    ...
    return std::views::iota(NodeId{0}, static_cast<NodeId>(self().nodeCount())) |
           std::views::filter([this, from](NodeId to) { return hasEdge(from, to); }) |
           std::views::transform([this, from](NodeId to) { return std::make_pair(to, std::ref(getEdge(from, to))); });
  }
  ...
  auto allEdges() const noexcept {
    return std::views::iota(NodeId{0}, static_cast<NodeId>(self().nodeCount())) |
           std::views::transform([this](NodeId from) { return edgesFrom(from); }) |  //
           std::views::join;
  }
