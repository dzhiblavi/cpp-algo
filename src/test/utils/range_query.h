...
struct QueryHandle {
 ...
 private:
  ...
  friend class test::NaiveRangeQueryEngine;
};
...
class NaiveRangeQueryEngine {
 public:
  ...
  template <utility::NDView<T, NDims> NDView>
  explicit NaiveRangeQueryEngine(NDView source)
      : container_(shapeAs<T, NDims>(source)), view_(utility::asView(container_)) {
 ...
 private:
  template <size_t I>
  void updateRange(
      std::array<size_t, NDims>& ul, std::array<size_t, NDims>& ur, std::array<size_t, NDims>& idxs,
      const T& value) {
  ...
  template <size_t I>
  auto query(
      std::array<size_t, NDims>& ql, std::array<size_t, NDims>& qr,
      std::array<size_t, NDims>& idxs) {
  ...
  utility::NDVector<T, NDims> container_;
  utility::NDContainerView<decltype(container_)> view_;
};
