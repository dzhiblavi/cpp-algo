...
using ResolveKind = ::test::util::list::list<
    std::conditional_t<
        contains(Kind, GraphKind::kAdjMatrix),
        algo::graph::repr::AdjMatrixGraph<
            uint32_t, algo::graph::repr::empty_payload_t, StorageType, Direct, std::size_t>,
        int>,
    std::conditional_t<
        contains(Kind, GraphKind::kAdjList),
        algo::graph::repr::AdjListGraph<uint32_t, algo::graph::repr::empty_payload_t, Direct, std::size_t>, int>>;

template <
    GraphKind::UnderlyingType Kind,  //
    bool Direct,                     //
    std::size_t StorageReserve>
using ResolveStorage = ::test::util::list::concat<
...
template <
    ...
    std::size_t StorageReserve>
using ResolveDirect = ::test::util::list::concat<
    std::conditional_t<
        contains(Direct, DirectKind::kDirect), ResolveStorage<Kind, true, StorageReserve>, ::test::util::list::list<>>,
    std::conditional_t<
        contains(Direct, DirectKind::kIndirect), ResolveStorage<Kind, false, StorageReserve>,
        ::test::util::list::list<>>>;
...
template <
    ...
    DirectKind::UnderlyingType Direct,  //
    std::size_t StorageReserve>
using GraphTypes = ::test::util::list::filter<int, detail::ResolveDirect<Kind, Direct, StorageReserve>>;
