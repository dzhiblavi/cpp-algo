...
struct VisitChecker {
  static void Check(ValueOrError<ValueType, ErrorTypes...>& voe) {
    if constexpr (std::is_same_v<void, VisitType>) {
      EXPECT_DEATH(voe.Visit(overloaded{[]() {}, [](auto&&) {}}), "Visit\\(\\) called on an empty object");
    } else {
...
struct ValueConstructorTest {
  ...
  static void call(ValueOrError<ValueType, ErrorTypes...>*) {
    ...
    EXPECT_TRUE(collector.Equal(
        test::Op(test::Create, ValueType::Idx), test::Op(test::CONSTRUCT_MOVE, ValueType::Idx),
...
struct MakeErrorTest {
  ...
  static void TestMakeError(ValueOrError<ValueType, ErrorTypes...>*, ErrorType*) {
    ...
    EXPECT_TRUE(collector.Equal(
        test::Op(test::Create, ErrorType::Idx), test::Op(test::CONSTRUCT_MOVE, ErrorType::Idx),
...
struct CreateBase {
  ...
  template <typename ValueType, typename... ErrorTypes>
  static ValueOrError<ValueType, ErrorTypes...> Create(ValueOrError<ValueType, ErrorTypes...>*, EmptyMarker) {
    return {};
  }

  template <typename ValueType, typename... ErrorTypes, typename T>
  static ValueOrError<ValueType, ErrorTypes...> Create(ValueOrError<ValueType, ErrorTypes...>*, ValueMarker<T>) {
    return T{};
  }

  template <typename ValueType, typename... ErrorTypes, typename T>
  static ValueOrError<ValueType, ErrorTypes...> Create(ValueOrError<ValueType, ErrorTypes...>*, ErrorMarker<T>) {
    return MakeError<T>();
  }

  template <typename ValueType, typename... ErrorTypes>
  static ValueOrError<ValueType, ErrorTypes...> CreateEmpty(ValueOrError<ValueType, ErrorTypes...>* ptr) {
    return Create(ptr, EmptyMarker{});
  }

  template <typename ValueType, typename... ErrorTypes, typename T>
  static ValueOrError<ValueType, ErrorTypes...> CreateOther(ValueOrError<ValueType, ErrorTypes...>*, ValueMarker<T>) {
    return MakeError<typename First<ErrorTypes...>::type>();
  }

  template <typename ValueType, typename... ErrorTypes, typename T>
  static ValueOrError<ValueType, ErrorTypes...> CreateOther(ValueOrError<ValueType, ErrorTypes...>*, ErrorMarker<T>) {
    return ValueType{};
  }
...
struct AssignmentsTest : CreateBase {
  ...
  static void call(list::list<ValueOrError<ValueType1, ErrorTypes1...>, ValueOrError<ValueType2, ErrorTypes2...>>*) {
    ...
    TestAssign(
        /* from = */ CreateEmpty(p1),
        /* to =   */ CreateEmpty(p2), -1, test::None, 0);
    ...
    if constexpr (!std::is_same_v<void, ValueType2>) {
      ...
      TestAssign(
          /* from = */ CreateEmpty(p1),
          /* to =   */ Create(p2, ValueMarker<ValueType2>{}), -1, test::None, 0,
          test::Op(test::Destroy, ValueType2::Idx));
    }
    (TestAssign(
         /* from = */ CreateEmpty(p1),
         /* to =   */ Create(p2, ErrorMarker<ErrorTypes2>{}), -1, test::None, 0,
    ...
    if constexpr (!std::is_same_v<void, ValueType1> && !std::is_same_v<void, ValueType2>) {
      ...
      TestAssign(
          ...
          /* to =   */ CreateEmpty(p2), ValueType1::Idx, test::CONSTRUCT_COPY, 1);
    }
    (TestAssign(
         /* from = */ Create(p1, ErrorMarker<ErrorTypes1>{}),
         /* to =   */ CreateEmpty(p2), ErrorTypes1::Idx, test::CONSTRUCT_COPY, 1),
     ...);
    ...
    if constexpr (!std::is_same_v<void, ValueType1> && !std::is_same_v<void, ValueType2>) {
      TestAssign(
          /* from = */ Create(p1, ValueMarker<ValueType1>{}),
          /* to =   */ Create(p2, ValueMarker<ValueType2>{}), ValueType1::Idx, test::ASSIGN_COPY, 1);
    }
    (TestAssign(
         /* from = */ Create(p1, ErrorMarker<ErrorTypes1>{}),
         /* to =   */ Create(p2, ErrorMarker<ErrorTypes1>{}), ErrorTypes1::Idx, test::ASSIGN_COPY, 1),
     ...);
    ...
    if constexpr (!std::is_same_v<void, ValueType1> && !std::is_same_v<void, ValueType2>) {
      if constexpr (sizeof...(ErrorTypes1) > 0) {
        ...
        TestAssign(
            ...
            /* to =   */ Create(p2, ValueMarker<ValueType2>{}), OtherIndex(p1, ValueMarker<ValueType1>{}),
            test::CONSTRUCT_COPY, 1, test::Op(test::Destroy, ValueType2::Idx));
        (TestAssign(
             /* from = */ CreateOther(p1, ErrorMarker<ErrorTypes1>{}),
             /* to =   */ Create(p2, ErrorMarker<ErrorTypes1>{}), OtherIndex(p1, ErrorMarker<ErrorTypes1>{}),
  ...
  template <typename ValueType1, typename... ErrorTypes1, typename ValueType2, typename... ErrorTypes2, typename... Ops>
  static void TestAssign(
      ValueOrError<ValueType1, ErrorTypes1...> v_from, ValueOrError<ValueType2, ErrorTypes2...> v_to, int index_from,
...
TEST(ValueConstructorTest, Correctness) {
  using Types = test::Types<test::RememberLastOp<0>, test::RememberLastOp<0>, int, char, std::string, const char*>;
  test::InstantiateAndCall<ValueConstructorTest>(Types{});
}

TEST(MakeErrorTest, Correctness) {
  using Types = test::TypesWithErrors<
      test::RememberLastOp<0>, test::RememberLastOp<0>, test::RememberLastOp<1>, test::RememberLastOp<2>>;
